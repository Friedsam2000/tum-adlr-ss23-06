...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"models/PPO_MLP_1"):
        os.makedirs(f"models/PPO_MLP_1")
    ...
    while model.num_timesteps < MAX_TIMESTEPS:
        ...
        if reward_mean > best_reward:
            ...
            model.save(f"models/PPO_MLP_1/{model.num_timesteps}")

            # upload the model to the bucket
            blob = bucket.blob(f"data_Matthias/models/PPO_MLP_1/{model.num_timesteps}.zip")
            blob.upload_from_filename(f"models/PPO_MLP_1/{model.num_timesteps}.zip")
        ...
        if log_save_counter%40 == 0:
            ...
            blob = bucket.blob(f"data_Matthias/logs/PPO_MLP_1/{latest_log}")

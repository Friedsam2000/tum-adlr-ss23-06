...
if __name__ == "__main__":
    ...
    policy_kwargs = dict(net_arch=[512, 128, 32])

    # Initialize DQN agent
    model = DQN("MlpPolicy", env, policy_kwargs=policy_kwargs, verbose=1, tensorboard_log="logs", device=device, buffer_size=35000, learning_starts=30000, batch_size = 128)
    ...
    while model.num_timesteps < MAX_TIMESTEPS:
        ...
        if reward_mean > best_reward:
            ...
            blob = bucket.blob(f"basic_environment/reinforcementPretrained/models/DQN_{len(logs_folders)}_0/{model.num_timesteps}.zip")
        ...
        blob = bucket.blob(f"basic_environment/reinforcementPretrained/logs/DQN_{len(logs_folders)}_0/{latest_log}")

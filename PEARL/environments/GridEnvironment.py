...
class CustomEnv_rc(gym.Env):
    ...
    def __init__(self, grid_size, img_size=(48, 48), render_size=(480, 480), num_last_agent_pos=2):
        ...
        self.img_size = img_size
        self.render_size = render_size
        super(CustomEnv_rc, self).__init__()
        # Define action and observation space
        self.action_space = spaces.Discrete(4)
        # Example for using image as input:
        self.observation_space = spaces.Box(low=0, high=255, shape=(img_size[0], img_size[1], 3), dtype=np.uint8)
        self.obstacles_color = (np.random.randint(0,255), np.random.randint(0,255), np.random.randint(0,255))
	
    def reset(self):
        self.done = False
        #new random obstacles color 
        self.obstacles_color = (np.random.randint(0,255), np.random.randint(0,255), np.random.randint(0,255))
        # create a deque to store the last agent positions
        self.last_agent_positions = deque(maxlen=self.num_last_agent_pos)
        # create history of agent positions and set to -1
        ...
        # set the agent position randomly
        self.agent_position = [np.random.randint(0, self.grid_size[0]), np.random.randint(0, self.grid_size[1])]

        # set the goal position randomly
        self.goal_position = [np.random.randint(0, self.grid_size[0]), np.random.randint(0, self.grid_size[1])]
        
        #enforce min distance between goal and start position
        while not(np.linalg.norm(np.array(self.agent_position) - np.array(self.goal_position)) > 5):
            self.goal_position = [np.random.randint(0, self.grid_size[0]), np.random.randint(0, self.grid_size[1])]
...
class CustomEnv_rc_fp(gym.Env):
    ...
    def __init__(self, grid_size, img_size=(48, 48), render_size=(480, 480), num_last_agent_pos=2):
        ...
        self.img_size = img_size
        self.render_size = render_size
        super(CustomEnv_rc_fp, self).__init__()
        # Define action and observation space
        self.action_space = spaces.Discrete(4)
        # Example for using image as input:
        self.observation_space = spaces.Box(low=0, high=255, shape=(img_size[0], img_size[1], 3), dtype=np.uint8)
...
class CustomEnv_sr_fp(gym.Env):
    ...
    def __init__(self, grid_size, img_size=(48, 48), render_size=(480, 480), num_last_agent_pos=2):
        ...
        self.img_size = img_size
        self.render_size = render_size
        super(CustomEnv_sr_fp, self).__init__()
        # Define action and observation space
        self.action_space = spaces.Discrete(4)
        # Example for using image as input:
        self.observation_space = spaces.Box(low=0, high=255, shape=(img_size[0], img_size[1], 3), dtype=np.uint8)
